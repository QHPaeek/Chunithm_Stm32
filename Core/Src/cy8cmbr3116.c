...
void Sensor_Cfg(I2C_HandleTypeDef *hi2cx)
{
	...
	while(HAL_I2C_Mem_Write(hi2cx,SENSOR_ADDR,CTRL_CMD,I2C_MEMADD_SIZE_8BIT,&sensor_cmd,1,100)!=HAL_OK);
	osDelay(500);
	...
	while(HAL_I2C_Mem_Read(hi2cx,SENSOR_ADDR,CTRL_CMD_STATUS,I2C_MEMADD_SIZE_8BIT,&sensor_cmd_status,1,100)!=HAL_OK);
		...
		else
		{
			while(1)
			{
				...
				while(HAL_I2C_Mem_Read(hi2cx,SENSOR_ADDR,0x89,I2C_MEMADD_SIZE_8BIT,&error_status,1,100)!=HAL_OK);
