...
//	uint8_t DMA_key_Buffer[2][32] = {0};
//	//HAL_I2C_Mem_Read(&hi2c1,SENSOR_ADDR,KEY_ADDR[i],I2C_MEMADD_SIZE_8BIT,&mem_temp,1,100);
//	HAL_I2C_Mem_Read_DMA(&hi2c1,SENSOR_ADDR,0xba,I2C_MEMADD_SIZE_8BIT,DMA_key_Buffer[0],32);
//	HAL_I2C_Mem_Read_DMA(&hi2c1,SENSOR_ADDR,0xba,I2C_MEMADD_SIZE_8BIT,DMA_key_Buffer[1],32);
  while(1)
  {
	osDelay(5);
...
//			key_status[i] = DMA_key_Buffer[0][2*i+1];
//			key_status[i+16] = DMA_key_Buffer[1][2*i+1];
//		}
		...
		if(Air_scan_flag){
			CDC_Transmit(0, CDC_Trans_Buffer, 42);
		}
		else{
